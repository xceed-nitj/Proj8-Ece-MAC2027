...
// Design 1 — classic centred avatar card.
// Card is a link only when a profile URL exists; external URLs must use <a>
// (react-router's <Link> treats absolute URLs as app paths and breaks).
function SpeakerCard({ speaker, apiUrl }) {
  const [imgFailed, setImgFailed] = useState(false);
  const imgSrc = resolveImgSrc(speaker.ImgLink, apiUrl);

  const card = (
    <div className="w-full h-full bg-white border border-blue-100 hover:border-blue-200 rounded-2xl shadow-sm hover:shadow-lg hover:shadow-blue-100/60 flex flex-col items-center justify-center p-5 min-h-[287px] transition-all duration-200">
      {imgSrc && !imgFailed ? (
        <img
          src={imgSrc}
          alt={speaker.Name}
        ...
        <div className="w-32 h-32 rounded-full bg-blue-50 text-blue-400 flex items-center justify-center mb-4 text-4xl font-bold font-poppins">
          {(speaker.Name || "?").trim().charAt(0).toUpperCase()}
        </div>
      )}
      <p className="font-poppins text-[17px] font-bold text-slate-800 text-center">{speaker.Name}</p>
      {speaker.Designation && (
        <p className="font-sans text-[12px] font-medium text-slate-500 text-center mt-0.5">{speaker.Designation}</p>
      )}
      <p className="font-sans text-[12px] font-semibold text-blue-600 text-center mt-0.5">{speaker.Institute}</p>
    </div>
  );
...
function Speaker1(props) {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        // Hide speakers whose Feature toggle is off in the admin panel;
        // missing sequence values sort to the end instead of randomly.
  ...
  return (
    <PageShell confid={confid} title={pageTitle} breadcrumbLabel="Speakers" maxWidth="max-w-7xl">
      {/* Tabs — Invited tab only appears when invited speakers exist */}
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 animate-pulse">
          {[...Array(8)].map((_, i) => (
            ...
            >
              <div className="w-32 h-32 rounded-full bg-blue-50 mb-4" />
